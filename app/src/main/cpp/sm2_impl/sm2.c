					  ...
					  mp_int * mp_XA, mp_int * mp_YA)
{
	...
	ZA_SRC_Buff = (unsigned char *)malloc(lenZA_SRC + MAX_STRLEN) ;
	if (NULL == ZA_SRC_Buff)
	{
		ret =  ERR_MEM_ALLOC;
		goto END;
	...
	pM_A = (unsigned char *) malloc(32+lenSrc+MAX_STRLEN);
	if (NULL == pM_A)
	{
		ret =  ERR_MEM_ALLOC;
		goto END;
...
int Byte2Mp_Int(mp_int * mp_tar, unsigned char *src_byte, unsigned long lenSrc)
{
	...
	src_strbuff = ( char *)malloc(lenSrc*2 + MAX_STRLEN);
	if (NULL == src_strbuff)
	{
...
BYTE *KDF(BYTE *str1, long klen, long strlen1)
{
    unsigned int ct = 0x00000001;
    long group_number = ((klen + (HASH_BIT_LENGTH - 1)) / HASH_BIT_LENGTH);
    BYTE *H = (BYTE *)malloc(group_number * HASH_BYTE_LENGTH);
    int i;
    
    for (i = 0; i < group_number; i++)
    {
        //ct∏¥÷∆µΩ◊÷∑˚¥Æ◊Ó∫Û£¨big-endian
...
int KDFwithSm3(unsigned char * kdfOutBuff, unsigned char * Z_in, unsigned long ulZlen, unsigned long klen ) {
    BYTE *t = KDF(Z_in, klen * 8, ulZlen);
    memcpy(kdfOutBuff, t, klen);
    return MP_OKAY;
}
...
int KDFwithSm3(unsigned char * kdfOutBuff, unsigned char * Z_in, unsigned long ulZlen, unsigned long klen )
{
    printf("KDF ulZlen = %lu,klen = %lu\n", ulZlen,klen );
    BYTE_print(Z_in,ulZlen);
    ...
    printf("max_iter = %d mod = %lu\n",max_iter,mod);
    
	for (ct = 1; ct <= max_iter ; ct++)
	{	
        printf("run in looper");
	...
	memcpy(pZ+ulZlen, ct_un_buff, len_ct_unbuff);
    printf("pz = ");
    BYTE_print(pZ,ulZlen + 4);
	sm3(pZ, ulZlen + 4, tmp_buff);
    printf("tmp_buff = ");
    BYTE_print(tmp_buff,mod);
...
//	unsigned char C1[65] = {0};
	...
	pC2 = (unsigned char *)malloc(C2_len+10);
	if (NULL == pC2)
	{
		return ERR_MEM_ALLOC;
	}
...
#endif
	ptmp = (unsigned char *)malloc(tmpX2Len * 3);
	if (NULL == ptmp)
	{
		ret = ERR_MEM_ALLOC;
		goto END;
	...
	pout = (unsigned char *)malloc(C2_len + 10);
	if(NULL == pout)
	{
		ret = ERR_MEM_ALLOC;
		goto END;
	...
	ptmp = (unsigned char *)malloc(C2_len + tmpX2Len + tmpY2Len + 100);
	if (NULL == ptmp)
	{
		ret = ERR_MEM_ALLOC;
		goto END;
...
//    rand_k[] = "4C62EEFD6ECFC2B95B92FD6C3D9575148AFA17425546D49018E5388D49DD7B4F";//标准的随机数
	...
	do 
	{
		...
		memcpy(ptmp, tmpX2Buff, tmpX2Len);
		memcpy(ptmp+tmpX2Len, tmpY2Buff, tmpY2Len);
		pout =(unsigned char *) malloc(plainLen + 10);
		if(NULL == pout)
		{
			ret = ERR_MEM_ALLOC;
			goto END;
	...
	// C2 = M XOR t
	C2_buf = (unsigned char *)malloc(plainLen + 10);
	if (NULL == C2_buf)
	{
		//delete [] C2_buf;
		ret = ERR_MEM_ALLOC;
	...
	ptmp = (unsigned char *)malloc(plainLen + tmpX2Len + tmpY2Len + 100);
	if (NULL == ptmp)
	{
		ret = ERR_MEM_ALLOC;
		goto END;
